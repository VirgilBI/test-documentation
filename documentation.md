...
### Definition: Calculates the total amount each customer has spent, excluding payments made with coupons and test users.
### Sample Query:
```sql
SELECT 
    c.id AS customer_id,
...
8. Test Users: Exclude test users from all analyses unless explicitly asked to include them. A test user is defined as having billable_status = false in the jaffle_shop.billable_entities table.