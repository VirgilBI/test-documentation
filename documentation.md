...
## Total Spend Per Customer (Excluding Test Users)
### Definition: Calculates the total amount each billable customer has spent, excluding payments made with coupons and test users.
### Sample Query:
```sql
SELECT 
    c.id AS customer_id,
...
## Monthly Active Users (Excluding Test Users)
### Definition: The number of billable users that make at least one purchase per month.
### Sample Query:
```sql
SELECT
...
## Revenue by Line Item and Month (Excluding Test Users)
### Definition: The total revenue generated by each line item per month from billable users, including orders without specific line items.
### Sample Query:
```sql
